 ...
 */
export function registerOwnedSessionEventType() {
    try {
        ...
        KNOWN_SESSION_EVENT_TYPES.add(OWNED_EVENT);
    }
    catch (error) {
        // 未来核心若冻结该集合，此处降级为告警：插件功能不受影响，
        // 只是含本插件事件的旧日志仍会被无本插件上下文的读者拒绝。
        console.warn(`[${name}] register owned session event type failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
