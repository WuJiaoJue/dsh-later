...
function frozenBarSegments(item, frozenLeftMs, firstSeen) {
  const end = Date.parse(item.scheduled_at);
  const now = Date.parse(item.paused_at ?? "");
  const frozenNow = Number.isNaN(now) ? end - Math.floor(frozenLeftMs / 1e3) * 1e3 : now;
...
function MultiBar({
  ...
  const { totalRatio, count } = frozen ? frozenBarSegments(item, frozenLeftMs ?? 0, firstSeen) : barSegments(item, now, firstSeen);
