 ...
 * - 其余：`created_at` → 首见时刻
 */
export function barWindow(item, end, now, firstSeen) {
    let start;
    if (item.kind === 'after') {
...
export function barSegments(item, now, firstSeen) {
 ...
 */
export function frozenBarSegments(item, frozenLeftMs, firstSeen, renderedAt) {
    const windowSec = item.window_seconds ?? item.after_seconds;
 ...
 */
export function planBarAnimation(totalMs, elapsedMs) {
